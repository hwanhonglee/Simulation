  ...
  autoware_auto_perception_msgs::msg::TrafficLightRoi & tl_roi)
{
  ...
  // for roi.x_offset and roi.y_offset
  {
    ...
    // HH_240927
    // 회전(Quaternion) 값 출력
    tf2::Quaternion rotation = tf_camera2tl.getRotation();
    std::cout << "A. Rotation (Quaternion): "
              << rotation.x() << ", "
              << rotation.y() << ", "
              << rotation.z() << ", "
              << rotation.w() << std::endl;
    ...
    // 위치(Vector3) 값 출력
    tf2::Vector3 translation = tf_camera2tl.getOrigin();
    std::cout << "B. Translation (Vector3): "
              << translation.x() << ", "
              << translation.y() << ", "
              << translation.z() << std::endl;

    // max vibration
    const double max_vibration_x = std::sin(config.max_vibration_yaw * 0.5) * tf_camera2tl.getOrigin().z() + config.max_vibration_width * 0.5;
    const double max_vibration_y = std::sin(config.max_vibration_pitch * 0.5) * tf_camera2tl.getOrigin().z() + config.max_vibration_height * 0.5;
    ...
    std::cout << "1. right_down point3d: " << point3d.x << ", " << point3d.y << ", " << point3d.z << std::endl;   // HH_240927, Traffic Light postion in ll2 map
    ...
    std::cout << "2. right_down point2d: " << point2d << std::endl;       // HH_240927, Projection Traffic Light position (in ll2 map) 3D to 2D Image 

    tl_roi.roi.x_offset = point2d.x;
    tl_roi.roi.y_offset = point2d.y;
    std::cout << "3. tl_roi.roi.offset: " << tl_roi.roi.x_offset << ", " << tl_roi.roi.y_offset << std::endl;   // HH_240927 
  }
  ...
  // for roi.width and roi.height
  {
    ...
    point3d.y = tf_camera2tl.getOrigin().y() - max_vibration_y;
    point3d.z = tf_camera2tl.getOrigin().z() - max_vibration_z;
    std::cout << "4. left_down point3d: " << point3d.x << ", " << point3d.y << ", " << point3d.z << std::endl;   // HH_240927, Traffic Light postion in ll2 map
    ...
    cv::Point2d point2d = calcRawImagePointFromPoint3D(pinhole_camera_model, point3d);
    roundInImageFrame(pinhole_camera_model, point2d);
    std::cout << "5. left_down point2d: " << point2d << std::endl;       // HH_240927 

    // HH_241008
    tl_roi.roi.width = point2d.x - tl_roi.roi.x_offset;
    // tl_roi.roi.height = point2d.y - tl_roi.roi.y_offset;
    // HH_241008
    tl_roi.roi.height = tl_roi.roi.y_offset - point2d.y;
    std::cout << "6. tl_roi.roi: " << tl_roi.roi.width << ", " << tl_roi.roi.height << std::endl;   // HH_240927 
