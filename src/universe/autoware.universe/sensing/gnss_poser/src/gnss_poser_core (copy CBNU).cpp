    ...
    std::make_shared<autoware_sensing_msgs::msg::GnssInsOrientationStamped>())
{
  ...
  // Subscribe
  nav_sat_fix_sub_ = create_subscription<sensor_msgs::msg::NavSatFix>("fix", rclcpp::QoS{1}, std::bind(&GNSSPoser::callbackNavSatFix, this, std::placeholders::_1)); //HH_230223 //fix
  autoware_orientation_sub_ = create_subscription<autoware_sensing_msgs::msg::GnssInsOrientationStamped>("autoware_orientation", rclcpp::QoS{1}, std::bind(&GNSSPoser::callbackGnssInsOrientationStamped, this, std::placeholders::_1));
  //HH_230419
  ego_heading_sub_ =
    create_subscription<novatel_gps_msgs::msg::NovatelHeading2>("/sensing/gnss/heading2", rclcpp::QoS{1},std::bind(&GNSSPoser::heading2callback, this, std::placeholders::_1));

  // Publish
  pose_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>("gnss_pose", rclcpp::QoS{1});
  pose_cov_pub_ = create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>( //HH_230413 // GNSS Pose
    "gnss_pose_cov", rclcpp::QoS{1});
  fixed_pub_ = create_publisher<tier4_debug_msgs::msg::BoolStamped>("gnss_fixed", rclcpp::QoS{1});
}

//HH_230419
void GNSSPoser::heading2callback(const novatel_gps_msgs::msg::NovatelHeading2::SharedPtr msg)
{
  ego_heading = M_PI + (msg->heading * M_PI / 180.0);

  q.setRPY(0, 0, ego_heading);
}

void GNSSPoser::callbackNavSatFix( //HH_230413 //Using GNSS value 
  const sensor_msgs::msg::NavSatFix::ConstSharedPtr nav_sat_fix_msg_ptr)
{
  ...
  //HH_230414
  gnss_base_pose_msg.pose.position.x = gnss_base_pose_msg.pose.position.x - 361928.920643;
  gnss_base_pose_msg.pose.position.y = gnss_base_pose_msg.pose.position.y - 4054315.496047;
  //HH_2390419
  gnss_base_pose_msg.pose.orientation.x = q.x();
  gnss_base_pose_msg.pose.orientation.y = q.y();
  gnss_base_pose_msg.pose.orientation.z = q.z();
  gnss_base_pose_msg.pose.orientation.w = q.w();

  tf2::toMsg(tf_map2base_link, gnss_base_pose_msg.pose);  

  // publish gnss_base_link pose in map frame //HH_230413 //sensing/gnss/pose
  pose_pub_->publish(gnss_base_pose_msg);

  // publish gnss_base_link pose_cov in map frame //HH_230413 //sensing/gnss/pose_with_covariance
  ...
  //HH_230414
  gnss_base_pose_cov_msg.pose.pose.position.x = gnss_base_pose_msg.pose.position.x - 361928.920643;
  gnss_base_pose_cov_msg.pose.pose.position.y = gnss_base_pose_msg.pose.position.y - 4054315.496047;
  //HH_230419
  gnss_base_pose_cov_msg.pose.pose.orientation.x = q.x();
  gnss_base_pose_cov_msg.pose.pose.orientation.y = q.y();
  gnss_base_pose_cov_msg.pose.pose.orientation.z = q.z();
  gnss_base_pose_cov_msg.pose.pose.orientation.w = q.w();
  ...
  publishTF("sensor_kit_base_link", gnss_base_frame_, gnss_base_pose_msg); //HH_230414
}
