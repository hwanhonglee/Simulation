  ...
  tier4_debug_msgs::msg::Float32MultiArrayStamped & diagnostic)
{
  ...
  MPCMatrix mpc_matrix = generateMPCMatrix(mpc_resampled_ref_traj, prediction_dt, current_pose, &mpc_data);
  ...
  const geometry_msgs::msg::Pose & current_pose, MPCData * data)
{
  ...
  // lateral_position_dob = data->nearest_pose.position.y;
...
MPCMatrix MPC::generateMPCMatrix(
  const trajectory_follower::MPCTrajectory & reference_trajectory, const double prediction_dt, const geometry_msgs::msg::Pose & current_pose, MPCData * data)
{
  ...
  // HJ_231208
  // state param
  double lateral_position_y_dob = current_pose.position.y;
  double time_gap_dob = m_ctrl_period;
  double heading_angle_dob = tf2::getYaw(data->nearest_pose.orientation);
  double current_steering_angle_dob = m_raw_steer_cmd_pprev;
  double cmd_steering_angle_dob = m_raw_steer_cmd_prev;
  // dob param
  double x1_dob = lateral_position_y_dob;
  double x2_dob = heading_angle_dob;
  double x3_dob = current_steering_angle_dob;
  double u_dob = cmd_steering_angle_dob;

  // HJ_231208
  // /* Disturbance observer calculation */
  MatrixXd X_dob(3,1);
  MatrixXd X_dob_pre(3,1);
  MatrixXd K_o_dob(3,3);

  X_dob << x1_dob, x2_dob, x3_dob;
  X_dob_pre << x1_dob_pre, x2_dob_pre, x3_dob_pre;

  // K_o_dob << 0.001, 0.0, 0.0,
  //        0.0, 0.001, 0.0,
  //        0.0, 0.0, 0.001;

  K_o_dob << 0.0001, 0.0, 0.0,
             0.0, 0.0001, 0.0,
             0.0, 0.0, 0.0001;
  ...
  Eigen::MatrixXd Wd_hat = MatrixXd::Zero(3, 1);
  Eigen::MatrixXd Wd_hat_k = MatrixXd::Zero(3, 1);
  Eigen::MatrixXd Wd_hat_pre = MatrixXd::Zero(3, 1);

  Wd_hat_k = Wd_hat_pre + K_o_dob*time_gap_dob*(X_dob - Ad*X_dob_pre - Bd*u_dob_pre - time_gap_dob*Wd_hat_pre);
  // Wd_hat = Wd_hat_k;

  double Wd_hat_1 = Wd_hat_k(0);
  double Wd_hat_2 = Wd_hat_k(1);
  double Wd_hat_3 = Wd_hat_k(2);

  // Saturation to publish messages
  if (std::abs(Wd_hat_1) <= 0.00000001)
  {
      Wd_hat_1 = 0.00000001;
  }
  if (std::abs(Wd_hat_2) <= 0.00000001)
  {
      Wd_hat_2 = 0.00000001;
  }
  if (std::abs(Wd_hat_3) <= 0.00000001)
  {
      Wd_hat_3 = 0.00000001;
  }
  ...
  Wd_hat << Wd_hat_1, Wd_hat_2, Wd_hat_3;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (i == 0) {
      ...
      m.Wex.block(0, 0, DIM_X, 1) = Wd; // HJ_231208 // Wd // Wd_hat
    } else {
      ...
      m.Wex.block(idx_x_i, 0, DIM_X, 1) = Ad * m.Wex.block(idx_x_i_prev, 0, DIM_X, 1) + Wd; // HJ_231208 // Wd // Wd_hat
    }
