...
: Node("lane_departure_checker_node", options)
{
  ...
  // Publisher
  // HH_240415 Publish yaw deviation dot, lateral deviation dot
  deviation_lateral_dot_publisher_ = this->create_publisher<tier4_debug_msgs::msg::Float64Stamped>("~/lateral_dot", 1);
  deviation_yaw_dot_publisher_ = this->create_publisher<tier4_debug_msgs::msg::Float64Stamped>("~/yaw_dot", 1);
  yaw_rate_desire_publisher_ = this->create_publisher<tier4_debug_msgs::msg::Float64Stamped>("~/yaw_rate_desire", 1);
  curvature_publisher_ = this->create_publisher<tier4_debug_msgs::msg::Float64Stamped>("~/curvature", 1);
  // Diagnostic Updater
...
// HH_240415 
double LaneDepartureCheckerNode::calculateCurvature(const geometry_msgs::msg::Point& prev, const geometry_msgs::msg::Point& current, const geometry_msgs::msg::Point& next)
{
  // Extract x, y coordinates from poses
  const double x0 = prev.x;
  const double y0 = prev.y;
  const double x1 = current.x;
  const double y1 = current.y;
  const double x2 = next.x;
  const double y2 = next.y;
  ...
  const double cross_product = dx1 * dy2 - dy1 * dx2;
  
  // Calculate squared distance between points
  const double squared_distance = dx1 * dx1 + dy1 * dy1;
  
  // Calculate curvature
  const double curvature = 2.0 * cross_product / (squared_distance * sqrt(squared_distance));
...
void LaneDepartureCheckerNode::onTimer()
{
  ...
  // HH_240415
  // Calculate time derivative of yaw deviation
  double yaw_deviation_dot = (output_.trajectory_deviation.yaw - prev_yaw_deviation_) / dt;
  prev_yaw_deviation_ = output_.trajectory_deviation.yaw;

  // Calculate time derivative of lateral deviation
  double lateral_deviation_dot = (output_.trajectory_deviation.lateral - prev_lateral_deviation_) / dt;
  prev_lateral_deviation_ = output_.trajectory_deviation.lateral;

  tier4_debug_msgs::msg::Float64Stamped deviation_yaw_dot_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
  deviation_yaw_dot_msg.stamp = this->now(); // 현재 시간으로 타임스탬프 설정
  deviation_yaw_dot_msg.data = yaw_deviation_dot;
  deviation_yaw_dot_publisher_->publish(deviation_yaw_dot_msg);

  tier4_debug_msgs::msg::Float64Stamped deviation_lateral_dot_msg(rosidl_runtime_cpp::MessageInitialization::ZERO);
  deviation_lateral_dot_msg.stamp = this->now(); // 현재 시간으로 타임스탬프 설정
  deviation_lateral_dot_msg.data = lateral_deviation_dot;
  deviation_lateral_dot_publisher_->publish(deviation_lateral_dot_msg);
  
  // Calculate yaw rate, curvature
  double yaw_rate = 0.0;
  if (current_odom_) 
  {
    const double vx = current_odom_->twist.twist.linear.x;
    
    if (reference_trajectory_->points.size() >= 3) 
    {
      const auto& prev_point = reference_trajectory_->points[0].pose.position;
      const auto& current_point = reference_trajectory_->points[1].pose.position;
      const auto& next_point = reference_trajectory_->points[2].pose.position;
      
      // Calculate curvature
      const double curvature = calculateCurvature(prev_point, current_point, next_point);
      
      // Calculate yaw rate
      yaw_rate = vx * curvature;

      // Publish curvature
      tier4_debug_msgs::msg::Float64Stamped curvature_msg;
      curvature_msg.stamp = this->now();
      curvature_msg.data = curvature;
      curvature_publisher_->publish(curvature_msg);
    } 
    
    else {RCLCPP_WARN(get_logger(), "Reference trajectory does not have enough points for curvature calculation.");}
  } 
  else {RCLCPP_WARN(get_logger(), "Current odometry message is not available.");}

  // Publish yaw rate
  tier4_debug_msgs::msg::Float64Stamped yaw_rate_msg;
  yaw_rate_msg.stamp = this->now();
  yaw_rate_msg.data = yaw_rate;
  yaw_rate_desire_publisher_->publish(yaw_rate_msg);
  // HH_240415

  {
    const auto & deviation = output_.trajectory_deviation;
    debug_publisher_.publish<tier4_debug_msgs::msg::Float64Stamped>("deviation/lateral", deviation.lateral);
    debug_publisher_.publish<tier4_debug_msgs::msg::Float64Stamped>("deviation/yaw", deviation.yaw);
    debug_publisher_.publish<tier4_debug_msgs::msg::Float64Stamped>("deviation/yaw_deg", rad2deg(deviation.yaw));
  }
