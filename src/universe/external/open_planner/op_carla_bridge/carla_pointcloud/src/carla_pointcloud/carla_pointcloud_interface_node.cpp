...
void PointCloudInterface::processScan(const sensor_msgs::msg::PointCloud2::SharedPtr scanMsg)
{
	if (b_create_ex_)
	{
		...
		pcl::PointCloud<pcl::PointXYZI>::Ptr basic_cloud(new pcl::PointCloud<pcl::PointXYZI>);
		...
		for (const auto &p : input_cloud->points) {
			...
			basic.y = p.y;
			basic.z = p.z;
			basic.intensity = static_cast<float>(p.intensity);  // or scaled
			basic_cloud->points.push_back(basic);
		  }
		...
		// publish basic_cloud → for centerpoint etc
		auto ros_basic = std::make_unique<sensor_msgs::msg::PointCloud2>();
		pcl::toROSMsg(*basic_cloud, *ros_basic);
		ros_basic->header.frame_id = "sensor_kit_base_link";
		velodyne_points_pub_->publish(std::move(ros_basic));

		// publish input_cloud → for preprocessors
		auto ros_extended = std::make_unique<sensor_msgs::msg::PointCloud2>();
		pcl::toROSMsg(*input_cloud, *ros_extended);
		ros_extended->header.frame_id = "sensor_kit_base_link";
		velodyne_points_ex_pub_->publish(std::move(ros_extended));
	...
	else
	{
		...
		if (pcl_ros::transformPointCloud(tf_output_frame_, *scanMsg, transformed_cloud, *tf_buffer_))
		{
			...
			pcl::fromROSMsg(transformed_cloud, *extended_cloud);

			pcl::PointCloud<pcl::PointXYZI>::Ptr basic_cloud(new pcl::PointCloud<pcl::PointXYZI>);
			for (const auto &p : extended_cloud->points)
			{
				pcl::PointXYZI bp;
				bp.x = p.x;
				bp.y = p.y;
				bp.z = p.z;
				bp.intensity = static_cast<float>(p.intensity);  // 또는 normalize
				basic_cloud->points.push_back(bp);
			}

			basic_cloud->width = basic_cloud->points.size();
			basic_cloud->height = 1;
			basic_cloud->is_dense = true;

			auto basic_msg = std::make_unique<sensor_msgs::msg::PointCloud2>();
			pcl::toROSMsg(*basic_cloud, *basic_msg);
			basic_msg->header = transformed_cloud.header;

			// auto extended_msg = std::make_unique<sensor_msgs::msg::PointCloud2>();
			// pcl::toROSMsg(*extended_cloud, *extended_msg);
			// extended_msg->header = transformed_cloud.header;
			
			velodyne_points_localization->publish(*basic_msg);
			velodyne_points_perception->publish(*basic_msg);
		}
...
: Node("carla_pointcloud_interface_node", node_options), tf_output_frame_("base_link"), b_create_ex_(true) // HH_250410 false
{
