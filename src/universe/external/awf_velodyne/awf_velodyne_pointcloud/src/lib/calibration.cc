...
#ifdef HAVE_NEW_YAMLCPP
  if (const auto max_node = node[MAX_INTENSITY]; max_node && max_node.IsDefined()) {
    float max_intensity_float;
    max_node >> max_intensity_float;
    correction.second.max_intensity = floor(max_intensity_float);
  } else {
    correction.second.max_intensity = 255;
  }

  if (const auto min_node = node[MIN_INTENSITY]; min_node && min_node.IsDefined()) {
    float min_intensity_float;
    min_node >> min_intensity_float;
